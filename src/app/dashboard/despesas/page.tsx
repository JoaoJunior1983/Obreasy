      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-6">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 text-gray-300 hover:text-white hover:bg-slate-800/50 rounded-lg transition-all font-medium"
          >
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400">{obra.nome}</p>
          <p className="text-sm text-gray-500 mt-1">
            Total de despesas: {despesasFiltradas.length}
          </p>
        </div>

        {/* Cards - CAMADA ELEVADA */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
          <div className="bg-slate-800 p-6 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.4)]">
            <h2 className="text-lg font-semibold text-gray-300 mb-2">Total no Período</h2>
            <p className="text-3xl font-bold text-white">R$ {totalPeriodo.toLocaleString('pt-BR')}</p>
          </div>
          <div className="bg-slate-800 p-6 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.4)]">
            <h2 className="text-lg font-semibold text-gray-300 mb-2">Material/Outros</h2>
            <p className="text-3xl font-bold text-white">R$ {totalMaterialOutros.toLocaleString('pt-BR')}</p>
            <p className="text-sm text-gray-400 mt-1">{percentualMaterialOutros}% do orçamento</p>
          </div>
          <div className="bg-slate-800 p-6 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.4)]">
            <h2 className="text-lg font-semibold text-gray-300 mb-2">Mão de Obra</h2>
            <p className="text-3xl font-bold text-white">R$ {totalMaoDeObra.toLocaleString('pt-BR')}</p>
            <p className="text-sm text-gray-400 mt-1">{percentualMaoDeObra}% do orçamento</p>
          </div>
        </div>

        {/* Distribuição - CAMADA INTERMEDIÁRIA */}
        <div className="bg-slate-900/60 p-6 rounded-xl shadow-[0_4px_20px_rgb(0,0,0,0.3)] mb-8">
          <h2 className="text-xl font-semibold text-white mb-4">Distribuição de Gastos</h2>
          <div className="flex h-6 rounded-full overflow-hidden bg-slate-800/50">
            <div
        ...
        {/* Tabela de Despesas - CAMADA INTERMEDIÁRIA */}
        <div className="bg-slate-900/60 p-6 rounded-xl shadow-[0_4px_20px_rgb(0,0,0,0.3)]">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-800/50">
                    <th className="text-left py-3 text-gray-300 font-medium">Data</th>
                    <th className="text-left py-3 text-gray-300 font-medium">Tipo</th>
                    <th className="text-left py-3 text-gray-300 font-medium">Descrição</th>
                    <th className="text-left py-3 text-gray-300 font-medium">Valor</th>
                    <th className="text-left py-3 text-gray-300 font-medium">Profissional</th>
                    <th className="text-left py-3 text-gray-300 font-medium">Ações</th>
                  </tr>
                </thead>
                <tbody>
                  {despesasFiltradas.map(despesa => {
                    ...
                    return (
                      <tr key={despesa.id} className="border-b border-slate-800/30 hover:bg-slate-800/30 transition-colors">
                        <td className="py-3 text-gray-300">{new Date(despesa.data).toLocaleDateString('pt-BR')}</td>
                        <td className="py-3">
                        ...
                        <td className="py-3 text-gray-300">{despesa.descricao || '-'}</td>
                        <td className="py-3 font-semibold text-white">R$ {despesa.valor.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                        <td className="py-3 text-gray-400">{profissional?.nome || '-'}</td>
                        <td className="py-3">
